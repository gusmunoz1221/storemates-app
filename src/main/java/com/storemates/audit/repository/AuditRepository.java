...
public interface AuditRepository extends JpaRepository<ProductEntity,Long> {
    @Query(value = """
        SELECT
            ...
            TO_CHAR(TO_TIMESTAMP(r.timestamp / 1000), 'DD/MM/YYYY HH24:MI:SS') AS fechaHora
        FROM products_aud p
        JOIN revinfo_custom r ON p.rev = r.id
        WHERE TO_TIMESTAMP(r.timestamp / 1000) BETWEEN :start AND :end
        ORDER BY r.timestamp DESC
        """, nativeQuery = true)
    List<ProductAuditHistory> findAuditByDateRange(
